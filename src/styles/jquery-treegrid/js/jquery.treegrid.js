...
(function ($) {
    ...
    var methods = {
        ...
        initTree: function (options) {
            ...
            return this.each(function () {
                ...
                if (settings.getNodeNoParents.apply(this, [$(this)]).length) {
                    $this.find('.treegrid-indent').remove();
                    $this.find('.treegrid-expander').remove();
                }
                else {
        ...
        getChildNodes: function () {
            if ($(this).treegrid('getSetting', 'getChildNodes'))
    ...
    $.fn.treegrid.defaults = {
        ...
        getNodeNoParents: function (treegridContainer) {
            var result = $.grep(treegridContainer.find('tr'), function (element) {
                var templateClass = /treegrid-parent-([A-Za-z0-9_-]+)/;

                if (templateClass.test($(element).attr('class'))) {
                    var id = templateClass.exec($(element).attr('class'))[1];
                    var parentClass = "treegrid-" + id;
                    var childNotes = treegridContainer.find('tr.' + parentClass);
                    return childNotes.length == 0
                }
                return false;
            });
            return $(result);
